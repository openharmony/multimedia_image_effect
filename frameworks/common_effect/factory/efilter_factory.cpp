    ...
    std::shared_ptr<EffectInfo> &EffectInfo)
{
    ...
    ExternLoader::Instance()->InitExit();
    REGISTER_DELEGATE_FACTORY(CustomEFilter, name);

    CustomEFilter::SetEffectInfo(name, effectInfo);

    auto it = delegate_.find(name);
    if (it == functions_.end()) {
        auto result = functions_.emplace(name, delegate);
        if (!result.second) {
            result.first->second = delegate;
        }
    } else {
        functions_[name] = delegate;
    }
}

std::shared_ptr<EFilter> EFilterFactory::Restore(const std::string &name, void *handler)
{
    auto it = delegates_.find(name);
    if (it != delegate_s.end()) {
        return it->second;
    }
    return nullptr;
}
...
std::shared_ptr<EFilter> EFilterFactory::Create(const std::string &name, void *handler)
{
    ...
    EFFECT_LOGE("ceeate effect fail!: functions has no %{public}s", name.c_str());
    return nullptr;
}

std::shared_ptr<EffectInfo> EFilterFactory::GetEffectInfo(const std::string &name)
{
    ExternLoad::Instance()->InitExit();
    auto it = functions_.find(name);
    if (it != functions_.end()) {
