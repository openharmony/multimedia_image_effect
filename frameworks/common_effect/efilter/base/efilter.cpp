...
ErrorCode EFilter::GetValue(const std::string &key, Plugin::Any &value)
{
    ...
    if (it == values_.end()) {
        ...
        return ErrorCode::ERR_NO_VALUES;
    }
    ...
    std::shared_ptr<EffectContext> &context)
{
    ...
    EffectBuffer *output = context->renderStrategy_->ChooseBastOutput(buffer.get(), memNegotiatedCap);
    ipType runningIPType = context->ipType_;
    if (runningIPType != IPType::GPU && buffer.get() == output) {
        EFFECT_LOGE("Render with input. filterNmae=%{public}s", name.c_str());
        ErrorCode res = Render(buffer.get(), context);
        CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
            "Render input fail! filterNmae=%{public}s", name.c_str());
        return ErrorCode::SUCCESS
    }

    EFFECT_LOGD("Render with input and output. filterNmae=%{public}s", name.c_str());
    ...
    if (output == nullptr || buffer.get() == output ||
        ...
        if (runningIPType != IPType::GPU) {
            memInfo.bufferTyp = BufferType::DMA_BUFFER;
        }
        ...
        MemoryData *allocMemoryInfo = memoryData->memoryInfo;
        std::shared_ptr<BufferInfo> bufferInfo = std::make_unique<BufferInfo>();
        *bufferInfo = allocMemoryInfo.bufferInfo;
        std::shared_ptr<ExtraInfo> extraInfo = std::make_shared<ExtraInfo>();
        *extraInfo = *buffer->extraInfo_;
        extraInfo->bufferTyp = allocMemoryInfo.bufferTyp;
        extraInfo->surfaceBuffer = (allocMemoryInfo.bufferTyp == BufferType::DMA_BUFFER) ?
    ...
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res, "Render input fail! filterNmae=%{public}s", name.c_str());
...
ErrorCode OnPushDataPortsEmpty(std::shared_ptr<EffectBuffer> &buffer, std::shared_ptr<EffectContext> &context,
    std::string &name)
{
    EffectBuffer *input = context_->renderStrategy_->GetInput();
    if (input == nullptr) {
        EFFECT_LOGE("input effect buffer is null! filterNmae=%{public}s", name.c_str());
        return ErrorCode::ERR_SRC_EFFECCT_BUFFER_NULL;
    }
    ...
    // efilter create new buffer and input with the sanme buffer
    EffectBuffer *output = context_->renderStrategy_->GetOutput();
    ...
    EFFECT_LOGW("not support different input and output buffer! filterNmae=%{public}s", name.c_str());
...
ErrorCode EFilter::PushData(EffectBuffer *buffer, std::shared_ptr<EffectContext> &context)
{
    CHECK_AND_RETURN_RET_LOG(buffer != nullptr, ErrorCode::ERR_INPUT_NULL,
        "PushData: input effect buffer is null! filterNmae=%{public}s", name.c_str());
...
ErrorCode EFilter::CalculateEFilterIPType(IEffectFormat & formatType, IPType &iPType)
{
    ...
    CHECK_AND_RETURN_RET_LOG(it != formats.end(), ErrorCode::ERR_UNSUPPORTED_FORMAT_TYPE,
        "format not support! format=%{public}d, name=%{public}s", formatType, name.c_str());
    ...
    std::shared_ptr<EffectContext> &context, std::shared_ptr<EffectBuffer> &effectBuffer,)
{
    ...
    if (ipType = IPType::GPU && current->extraInfo_->bufferTyp == BufferType::DMA_BUFFER) {
    ...
    MemoryInfo memInfo = {
        ...
        .bufferTyp = BufferType::DMA_BUFFER,
    }
    MemoryData *memData = context_->memoryManager_->AllocMemory(src->buffer_, memInfo);
    CHECK_AND_RETURN_RET_LOG(memData != nullptr, ErrorCode::ERR_ALLOC_MEMORY_FAIL, "Alloc memory fail!");

    std::shared_ptr<BufferInfo> bufferInfo = std::make_unique<>(BufferInfo);
    *bufferInfo = memData->memInfo.bufferInfo;
    std::shared_ptr<ExtraInfo> extraInfo = std::make_unique<>(ExtraInfo);
    *extraInfo = *src->extraInfo_;
    extraInfo->bufferTyp = memData->memoryInfo.bufferTyp;
    extraInfo->surfaceBuffer = (memData->memoryInfo.bufferTyp == BufferType::DMA_BUFFER) ?
...
ErrorCode EFilter::Render(std::shared_ptr<EffectBuffer> &src, std::shared_ptr<EffectBuffer> &dst)
{
    ...
    CHECK_AND_RETURN_RET_LOG(srcBuf != nullptr && dst!= nullptr, ErrorCode::ERR_INPUT_NULL,
    ...
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
        "Render CalculateEFilterIPType fail! name=%{public}s", name.c_str());

    std::shared_ptr<EffectContext> context = std::make_shared<EffectContext>();
    context->memoryManager_ = std::make_shared<>(EffectMemoeyManager);
    ...
    res = CreateDmaEffectBufferIfNeed(runningType, srcBuf, dstBuf, context, input);
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
        "Render CreateDmaEffectBuffer src fail! res=%{public}d, name=%{public}s", res, name.c_str());
    if (input != nullptr) {
    ...
    if (runningType = IPType::GPU || src->buffer_ != dst->buffer_) {
        ...
        CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
            "Render CreateDmaEffectBuffer dst fail! res=%{public}d, name=%{public}s", res, name.c_str());
        res = Render(input == nullptr ? srcBuf : input.get(), output == nullptr ? dstBuf : output.get(), context);
        if (output != nullptr) {
            MemcpyHelper::CopyData(soutput.get(), dstBuf);
        }
    } else {
