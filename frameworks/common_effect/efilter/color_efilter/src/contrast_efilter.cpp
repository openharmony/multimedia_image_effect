...
const float ContrastEFilter::Paramter::RANGE[] = {-100.f, 100.f};
...
ErrorCode ContrastEFilter::SetValue(const std::string &key, Plugin::Any &value)
{
    ...
    if (contrast < Paramter::RANGE[0] || contrast > Paramter::RANGE[1]) {
        EFFECT_LOGW("the value is out of range! key=%{public}s, value=%{public}f, range=[%{public}f, %{public}f]",
            key.c_str(), contrast, Paramter::RANGE[0], Paramter::RANGE[1]);
            *contrastPtr = CLIP(contrast, Paramter::RANGE[0], Paramter::RANGE[1]);
    }
...
ErrorCode ContrastEFilter::Restore(const nlohmann::json &values)
{
    ...
    ErrorCode result = JsonHelper::GetFloatValue(values, Paramter::KEY_INTENSITY, contrast);
