...
namespace Media {
namespace Effect {
const std::string ColorEFilter::Paramter::KEY_INTENSITY = "KEY_INTENSITY";
...
{
    IPType ipType = context_->ipType_;
...
std::shared_ptr<EffectInfo> ColorEFilter::GetEffectInfo(const std::string &name)
{
    ...
    FilterFactory::Instance()->GetEffectInfo(name, info);
...
ErrorCode ColorEFilter::PreRender(IEffectFormat &format)
{
    ...
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
        "PreRender: CalculateEFilterIPType fail! name=%{public}s", name.c_str());
