...
const std::string CropEFilter::Paramter::KEY_REGION = "FILTER_REGION";
std::shared_ptr<EffectInfo> CropEFilter::info = nullptr;
namespace {
    ...
    Region *region)
{
    ...
    void *area = nullptr;
    ErrorCode res = CommUtils::GetValue(CropEFilter::Paramter::KEY_REGION, values, area);
    ...
    EFFECT_LOGW("CropEFilter x0=%{public}d, y0=%{public}d, x1=%{public}d, y1=%{public}d",
...
ErrorCode CropEFilter::Render(EffectBuffer *buffer, std::shared_ptr<EffectContext> &context)
{
    ...
    CHECK_AND_RETURN_RET_LOG(dataType == DataType::PIXEL_MAP || dataType == DataType::URI || DataType::PATH,
        ErrorCode::ERR_UNSUPPORTED_DATA_TYPE, "copy only support pixelMap uri path! dataType=%{public}d", dataType);
    ...
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res, "filter(%{public}s) render fail", name.c_str());
...
std::shared_ptr<EffectInfo> CropEFilter::GetEffectInfo(const std::string &name)
{
    ...
    info->category_ = Category::SHAPE_ADJUST;
    return info_;
}
