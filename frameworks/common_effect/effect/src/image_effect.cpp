...
public:
    IMRenderContext *context = nullptr;
    std::unique_ptr<ImageEffectReceiver> imageEffectReceiver_;
    ...
    EffectState effectContext_ = EffectState::IDLE;
};
...
void ImageEffect::Impl::InitEffectContext()
{
    ...
    effectContext_->memoryManager_ = std::make_shared<EffectMemoeyManager>();
...
void ImageEffect::Impl::CreatePipeline(std::vector<std::shared_ptr<EFilter>> &efilters)
{
    ...
    CHECK_AND_RETURN_LOG(res == ErrorCode::SUCCESS, res, "pipeline add filters fail!, res=%{public}d", res);

    res = pipeline_->LinkFilters(filtersToPipeline);
    CHECK_AND_RETURN_LOG(res == ErrorCode::SUCCESS, res, "pipeline link filter fail!, res=%{public}d", res);
}

struct EffectParamters {
    EffectParamters(std::shared_ptr<EffectBuffer> &srcEffectBuffer, std::shared_ptr<EffectBuffer> &dstEffectBuffer,
...
ImageEffect::~ImageEffect()
{
    ...
    impl_->imageEffectReceiver_ = nullptr;
...
{
    auto it = config.find(configIPType::IPTYPE);
    if (it == config.end()) {
...
ErrorCode ChooseIPType(const std::shared_ptr<EffectBuffer> &sinkBuffer,
    ...
    for (const auto &capability : caps) {
        ...
        if (it == formats.end()) {
            EFFECT_LOGE("effectFormat not support! effectFormat=%{public}d, name=%{public}s",
                effectFormat, capability->name.c_str());
...
ErrorCode StartPipelineInner(std::shared_ptr<PipelineCore> &pipeline, EffectParamters &effectParamters)
{
    ErrorCode res = pipeline->Prepare();
    FALSE_RETURN_MSG_E(res == ErrorCode::SUCCESS, res, "pipeline Prepare fail! res=%{public}d", res);
    
    IPType runningIPType;
    res = ChooseIPType(effectParamters.srcEffectBuffer_, effectParamters.effectContext_, effectParamters.config_,
        runningIPType);
    FALSE_RETURN_MSG_E(res == ErrorCode::SUCCESS, res, "choose running ip type fail! res=%{public}d", res);
    effectParamters.effectContext_->ipType_ = runningIPType;
    effectParamters.effectContext_->memoryManager_->SetIPType(runningIPType);
...
ErrorCode StartPipeline(std::shared_ptr<PipelineCore> &pipeline, EffectParamters &effectParamters)
{
    effectParamters.effectContext_->renderStrategy_->Init(effectParamters.srcEffectBuffer_.get(),
        effectParamters.dstEffectBuffer_.get());
    effectParamters.effectContext_->memoryManager_->Init(effectParamters.srcEffectBuffer_,
        effectParamters.dstEffectBuffer_);
    ErrorCode res = StartPipelineInner(pipeline, effectParamters);
    effectParamters.effectContext_->memoryManager_->Deinit();
    effectParamters.effectContext_->renderStrategy_->Deinit();
    effectParamters.effectContext_->capNegotiate_->ClearNegotiateResult();
    return res;
}
...
{
    if (uri.empty()) {
        EFFECT_LOGE("SetOutputUri: uri set to null");
        ClearDataInfo(outDateInfo_)
        return ErrorCode::SUCCESS;
    }
...
{
    if (path.empty()) {
        EFFECT_LOGE("SetOutputPath: uri set to null");
        ClearDataInfo(outDateInfo_)
        return ErrorCode::SUCCESS;
    }
    ...
    outDateInfo_.uri_ = std::move(path);
    ...
    std::shared_ptr<EffectBuffer> &dstEffectBuffer)
{
    ...
    // allow developers not to set the out papamter
    ...
    if (srcEffectBuffer->extraInfo_->dataType == DataType::PIXEL_MAP) {
        CHECK_AND_RETURN_RET_LOG(srcEffectBuffer->bufferInfo_->formatType_ == dstEffectBuffer->bufferInfo_->formatType_,
            ...
            srcEffectBuffer->bufferInfo_->dataType, dstEffectBuffer->bufferInfo_->formatType_);
    }
...
ErrorCode ImageEffect::Render()
{
    ...
    EffectParamters effectParamters(srcEffectBuffer, dstEffectBuffer, config_, impl_->effectContext_);
    res = StartPipeline(impl_->pipeline_, effectParamters);
    if (res != ErrorCode::SUCCESS) {
...
std::shared_ptr<ImageEffect> ImageEffect::Restore(std::string &info)
{
    ...
    for (auto &efilterInfo : efiltersInfo) {
        std::string name;
        CHECK_AND_CONTINUE_LOG(JsonHelper::GetStringValue(efiltersInfo, "name", name) == ErrorCode::SUCCESS,
...
{
    if (impl_->imageEffectReceiver_ == nullptr) {
        EFFECT_LOGE("impl imageEffectReceiver is nullptr!");
        return;
    }
    auto transform = impl_->imageEffectReceiver_->GetTransForm();
    if (transform == toProducerSurface_) {
        return;
    }
    toProducerSurface_ = transform;
...
void ImageEffect::ConsumerBufferAvailable(sptr<SurfaceBuffer>& buffer, const OHOS::Rect& damages, int64_t timestamp)
{
    ...
    sptr<SynFence> synFence = SynFence::INVALID_FENCE;
    ...
    auto ret = toProducerSurface_->RequestBuffer(outBuffer, synFence, requestConfig);
    if (ret != 0) {
        EFFECT_LOGE("RequestBuffer fail! ret=%{public}d", ret);
        return;
    }

    constexpr uint32_t waitForEver = -1;
    (void)synFence->Wait(waitForEver);
    ...
    if (isNeedCpy) {
        errno_t errorCode = memcpy_s(outBuffer->GetVirAddr(), outBuffer->GetSize(), buffer->GetVirAddr(),
            buffer->GetSize());
        CHECK_AND_RETURN_RET_LOG(errorCode == 0, "ConsumerBufferAvailable memcpy_s failed. %{public}d", errorCode);
    }
...
{
    inDateInfo_.dataType = DataType::SURFACE;
    outDateInfo_.dataType = DataType::SURFACE;
    if (fromProducerSurface_ != nullptr) {
        return fromProducerSurface_;
    }

    if (impl_->imageEffectReceiver_ == nullptr) {
        impl_->imageEffectReceiver_ == std::make_unique<ImageEffectReceiver>();
    }

    if (impl_->imageEffectReceiver_) {
        fromProducerSurface_ = impl_->imageEffectReceiver_->GetProducerSurface();
    }
    ...
    if (impl_->imageEffectReceiver_) {
        impl_->imageEffectReceiver_->SetConsumerListener(std::move(consumerListener));
    }
...
ErrorCode ImageEffect::Configure(const std::string &key, const Plugin::Any &value)
{
    auto configTypeIt = std::find_if(configTypeTab_.begin(), configTypeTab_.end(),
        [&key](const std::pair<std::string, ConfigType> &iem) {return item.first.compare(key) == 0; });
    ...
    switch (configType) {
        case ConfigType::IPTYPE: {
            ...
            auto it = std::find_if(runningTypeTab_.begin(), runningTypeTab_.end(),
                [&runningType](const std::pair<int32_t, std::vector<IPType>> &iem) {
    ...
    std::shared_ptr<EffectBuffer> &dstEffectBuffer)
{
    ...
    EFFECT_LOGI("input data set, start parse data info success. dataType=%{public}d", inDateInfo_.dataType_);

    if (outDateInfo_.dataType_ != DataType::UNKNOWN) {
        EFFECT_LOGI("output data set, start parse data info success. dataType=%{public}d", outDateInfo_.dataType_);
    ...
    bool isOutputData)
{
    switch (dataInfo.dataType_) {
        case DataType::PIXEL_MAP:
            return CommonUtils::LockPixelMap(dataInfo.pixelMap_);
        case DataType::SURFACE:
        case DataType::SURFACE_BUFFER:
        ...
        default:
            EFFECT_LOGE("dataType is not support! dataType=%{public}d", dataInfo.dataType_);
...
ErrorCode ImageEffect::SetExtraInfo(nlohmann::jspn res)
{
    extraInfo_ = res;
    return ErrorCode::SUCCESS;
}
...
{
    InitModuleFunc initModuleFunc = ExternLoader::Instance()->GetInitInitModuleFunc();
    if (initModuleFunc) {
        initModuleFunc();
    }
}
...
{
    DeinitModuleFunc deinitModuleFunc = ExternLoader::Instance()->GetDeinitInitModuleFunc();
    if (deinitModuleFunc) {
        deinitModuleFunc();
    }
}
