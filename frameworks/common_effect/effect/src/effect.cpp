...
ErrorCode Effect::InsertEFilter(const std::shared_ptr<EFilter> &efilter, uint32_t index)
{
    if (index > static_cast<uint32_t>(efilters_.size())) {
        EFFECT_LOGI("index is invalid! index=%{public}d, efiltersSize=%{public}zu", index, efilters_.size());
        return ErrorCode::ERR_INVALID_INDEX;
    }
    efilters_.emplace_back(efilters_.begin() + index, efilter);
    return ErrorCode::SUCCESS;
}

void Effect::InsertEFilter(const std::shared_ptr<EFilter> &efilter)
{
