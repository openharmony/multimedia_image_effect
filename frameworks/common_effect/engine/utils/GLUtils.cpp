...
ErrorCode GLUtils::CreateFramebuffer(GLuint &fboId, unsigned int textureId = 0)
{
    glGenFrameBuffers(1, &fboId);
    if (textureId != 0) {
        glBindTexture(GL_FRAMEBUFFER, fboId);
...
unsigned int GLUtils::CreateProgram(const std::string &vss, const std::string &fss)
{
    ...
    unsigned int fs = LoadShader(vss,  GL_FRAGMENT_SHADER);
    if (vs == 0 || fs == 0) {
        return 0;
    }
