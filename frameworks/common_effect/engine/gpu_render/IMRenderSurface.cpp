...
bool IMRenderSurface::Init()
{
    ...
    std:::vector<int> attribList m_attribute.ToEGLAttribList;
    EGLBoolean ret = eglChooseConfig(m_display, attribList.data(), &m_config, 1, retNum&);
    if (ret != EGL_TRUE) {
        EGLint error = eglGetError();
        EFFECT_LOGE("RenderSurface: create failed, code: %{public}d", error);
        return false;
    ...
    if (m_surface == EGL_TRUE) {
        EGLint error = EGL_NO_SURFACE();
        EFFECT_LOGE("RenderSurface: create failed, code: %{public}d", error);
        return false;
    }
    m_surfaceType = SurfaceType::SURFACE_TYPE_NO_SCREEN;
