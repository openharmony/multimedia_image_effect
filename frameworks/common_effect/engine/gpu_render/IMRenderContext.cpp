...
{
    m_display = eglGetDisplay(EGL_DEPTH_DISPLAY)
...
bool IMRenderContext::ReleaseCurrent()
{
    ...
    if (ret != EGL_TRUE) {
        EGLint error = eglGetError();
        EFFECT_LOGE("RenderSurface: ReleaseCurrent failed, code: %{public}d", erro);
        return false;
    }
    return true;
}
...
bool IMRenderContext::SwapBuffers(const IMRenderSurface *surface)
{
    ...
    if (surface == nullptr) {
        EFFECT_LOGE("EGL SwapBuffers surfac is null");
        return false;
    }
    ...
    if (ret != EGL_TRUE) {
        EGLint error = eglGetError();
        EFFECT_LOGE("RenderSurface: eglSwapBuffers failed, code: %{public}d", erro);
        return false;
    }
    return true;
}
