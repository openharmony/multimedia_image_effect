...
std::shared_ptr<MemoryData> HeapMemoryData::Alloc(MemoryInfo &memoryInfo)
{
    uint32_t size = memoryInfo.bufferInfo.len_;
    EFFECT_LOGI("HeapMemory::Alloc alloc addr=%{public}p", size);
...
std::shared_ptr<MemoryData> DmaMemory::Alloc(MemoryInfo &memoryInfo)
{
    ...
    auto *src = reinterpret_cast<Surface *>(memoryInfo.extra);

    BufferRequestConfig requestConfig = {
        .width = static_cast<int32_t>(bufferInfo.width_),
        .height = static_cast<int32_t>(bufferInfo.height_),
        .strideAlignment = 0x8, // set 0x8 as default value to alloc SurfaceBufferImpl
        .format = common_utils::SwitchToPixelFormat(bufferInfo.formatType_), // PixelFormat
        .usage = src == nullptr ?
        ...
        .transform = src == nullptr ? GraphicColorGamut::GRAPHIC_ROTATE_NONE : src->GetSurfaceBufferColorGamut(),
    };
    ...
    std::shared_ptr<DmaMemoryData> memoeyData = std::make_unique<DmaMemoryData>();
...
std::shared_ptr<MemoryData> DmaMemory::Alloc(MemoryInfo &memoryInfo)
{
    ...
    if (data == MAP_FAILED) {
        EFFECT_LOGE("SharedMemory::Alloc map failed, errno:%{public}d", errno);
                ::close(fd);
        return nullptr;
    }
...
std::unique_ptr<AbsMemory> EffectMemory::CreateMemory(BufferType bufferType)
{
    ...
    switch (bufferType) {
        ...
        case BufferType::DMA_BUFFER:
            res = std::make_unique<DmaMemoryData>();
            break;
