...
std::shared_ptr<uint8_t[]> MemoeyManager::AllocMemoey(size_t size)
{
    EFFECT_LOGI("AllocMemory size=%{public}zu", size);
    CHECK_AND_RETURN_RET_LOG(size <= MAX_RAM_SIZE && size > 0, nullptr,
        "data size out of range! data.szie=%{public}zu", size);
...
void MemoeyManager::ReleaseMemoey(std::shared_ptr<uint8_t[]> &data)
{
