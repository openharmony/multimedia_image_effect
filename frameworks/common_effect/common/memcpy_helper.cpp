...
void MemcpyHelper::CopyData(CopyInfo &src, CopyInfo &dst)
{
    ...
    if (rowCount * dstRowStride > dstBufferLen || rowCount * srcRowStride > srcBufferLen) {
        EFFECT_LOGD("Out of buffer available range! Copy fail! srcH=%{public}d, srcFormat=%{public}d, "
    ...
    for (uint32_t i = 0; i < rowCount; i++) {
        errno_t ret = memcpy_s(dstBuffer + i * dstRowStride, srcBuffet + i * srcRowStride, count);
        if (ret != 0) {
...
{
    CHECK_AND_RETURN_LOG(src != nullptr, "Input src effect is null! src=%{public}p", src);
    CopyInfo srcCopyInfo;
    CreateCopyInfoByEffectBuffer(src, srcCopyInfo);
...
{
    CHECK_AND_RETURN_LOG(dst != nullptr, "Input dst effect is null! dst=%{public}p", dst);
    CopyInfo dstCopyInfo;
    CreateCopyInfoByEffectBuffer(dst, dstCopyInfo);
