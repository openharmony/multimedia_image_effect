...
const std::unordered_map<PixelFormat, IEffectFormat> CommonUtils::pixelFmtToEffectFmt_ = {
    { PixelFormat::RGBA_8888, IEffectFormat::RGBA_8888 },
...
const std::unordered_map<::PixelFormat, IEffectFormat> CommonUtils::surfaceBufferFmtToEffectFmt_ = {
    { ::PixelFormat::PIXEL_FMT_RGBA_8888, IEffectFormat::RGBA_8888 },
...
ErrorCode CommonUtils::LockPixelMap(PixelMap *pixelMap, std::shared_ptr<EffectBuffer> &effectBuffer)
{
    ...
    if (formatType == IEffectFormat::DEFAULT) {
        EFFECT_LOGE("pixelMap not support! pixelFormat=%{public}d", pixelMap->GetPixelFormat());
    ...
    EFFECT_LOGI("pixelMapInfos: width=%{public}d, height=%{public}d formatType=%{public}d, " \
        "rowStride=%{public}d, len=%{public}d, addr=%{public}p",
    ...
    extraInfo->bufferType = SwitchToEffectBuffType(pixelMap->GetAlloccatorType());
    ...
    std::shared_ptr<EffectBuffer> &effectBuffer, const DataType &dataType)
{
    ...
    bufferInfo->width_ = static_cast<uint32_t>(pixelMap->getWidth());
    bufferInfo->height_ = static_cast<uint32_t>(pixelMap->getHeight());
    bufferInfo->rowStride_ = static_cast<uint32_t>(pixelMap->getStride());
...
ErrorCode CommonUtils::ParseUri(std::string &uri, std::shared_ptr<EffectBuffer> &effectBuffer, bool isOutputData)
{
    if (isOutputData) {
        std::shared_ptr<BufferInfo> bufferInfo = std::make_unique<BufferInfo>();
        std::shared_ptr<ExtraInfo> extraInfo = std::make_unique<ExtraInfo>();
        extra->dataType = DataType::URI;
        extra->uri =std::move(uri);
    ...
    CHECK_AND_RETURN_RET_LOG(effectBuffer->extraInfo_ != nullptr, ErrorCode::ERR_EXTRA_INFO_NULL,
        "ParseUri: extraInfo_ is null! uri=%{public}s", uri.c_str());
    effectBuffer->extraInfo->dataType = DataType::URI;
    effectBuffer->extraInfo->URI = std::move(uri);
    ...
    bool isOutputData)
{
    if (isOutputData) {
        std::shared_ptr<BufferInfo> bufferInfo = std::make_unique<BufferInfo>();
        std::shared_ptr<ExtraInfo> extraInfo = std::make_unique<ExtraInfo>();
        extra->dataType = DataType::PATH;
        extra->path =std::move(path);
        effectBuffer = std::make_unique<EffectBuffer>(bufferInfo, nullptr, extraInfo);
        return ErrorCode::SUCCESS;
    }
    ...
    CHECK_AND_RETURN_RET_LOG(imageSource != nullptr, ErrorCode::ERR_CREATE_PIXELMAP_FAIL,
        "ImageSource: CreateImageSource fail! path=%{public}s, res=%{public}d", path.c_str(), errorCode);

    DeodeOptions options;
    std::unique_ptr<PixelMap> imageSource = ImageSource->CreatePixelMap(options, errorCode);
    CHECK_AND_RETURN_RET_LOG(imageSource != nullptr, ErrorCode::ERR_CREATE_PIXELMAP_FAIL,
        "CreatePixelMap fail! path=%{public}s, res=%{public}d", path.c_str(), errorCode);

    ErrorCode res = LockPixelMap(pixelMap.get(), effectBuffer);
    CHECK_AND_RETURN_RET_LOG(res == ErrorCode::SUCCESS, res,
        "ParsePath: lock pixel map fail! uri=%{public}s, res=%{public}d", uri.c_str(), res);

    CHECK_AND_RETURN_RET_LOG(effectBuffer->extraInfo_ != nullptr, ErrorCode::ERR_EXTRA_INFO_NULL,
        "ParsePath: extra info_ is null! uri=%{public}s", path.c_str());
...
IEffectFormat CommonUtils::SwitchToEffectFormat(PixelFormat pixelFormat)
{
    ...
    auto itr = pixelFmtToEffectFmt.find(pixelFormat);
    if (itr != pixelFmtToEffectFmt.end()) {
        formatType = itr->second;
    }
...
{
    ::PixelFormat pixelFormat = ::PixelFormat::PIXEL_FMT_RGBA_8888;
...
bool CommonUtils::EndWithJPG(const std::string &input)
{
    return StringHelp::EndsWithIgnoreCase(input, "jpg") || StringHelper::EndsWithIgnoreCase(input, "jpeg");
}
...
{
    EFFECT_LOGE("ModifyPixelMapProperty enter!");
    ...
    if (allocMemory != nullptr && allocMemory->memoryData_->memoryInfo.bufferType == bufferType) {
        ...
        std::unique_ptr<AbsMemory> memory = EffectMemoey::CreateMemory(bufferType);
    ...
    pixelMap->SetPixelsAddr(memoryData->data, context, memoryData->memoryIndo.bufferInfo.len_, allocatorType, nullptr);
