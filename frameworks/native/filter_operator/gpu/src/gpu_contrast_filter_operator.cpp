...
#include "gpu_contrast_filter_operator.h"
#include <string>
...
namespace Media {
namespace Effect {
REGISTER_FILTER_OPERATOR(Contrast, GpuContrastFilterOperator);
constexpr int MAX_CONTRAST = 100;

const std::vector<IEffectFormat> GpuContrastFilterOperator::FORMAT_TYPES = {
...
ErrorCode GpuContrastFilterOperator::InitMesh()
{
...
void GpuContrastFilterOperator::Unbind()
{
...
ErrorCode GpuContrastFilterOperator::PreDraw(uint32_t width, uint32_t height, std::map<std::string, Plugin::Any> &value)
{
...
ErrorCode GpuContrastFilterOperator::Release()
{
...
float GpuContrastFilterOperator::ParseContrast(std::map<std::string, Plugin::Any> &value)
{
    float contrast = 0.f;
...
ErrorCode GpuContrastFilterOperator::OnApplyRGBA8888(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    ...
    if (code != ErrorCode::SUCCESS) {
        EFFECT_LOGE("GpuContrastFilterOperator Render CreateTexture Failed");
        return code;
    }

    code = GLUtils::CreateFramebuffer(fbo_, texId_);
    if (code != ErrorCode::SUCCESS) {
        EFFECT_LOGE("GpuContrastFilterOperator Render CreateFramebuffer Failed");
        return code;
    }
    ...
    code  = InitMesh();
    if (code != ErrorCode::SUCCESS) {
        EFFECT_LOGE("GpuContrastFilterOperator Render InitMesh Failed");
        return code;
    }

    code  = PreDraw(width, height, value);
    if (code != ErrorCode::SUCCESS) {
        EFFECT_LOGE("GpuContrastFilterOperator Render PreDraw Failed");
        return code;
    }

    glDrawArrays(GL_TRIANGLE_STRIP, 0, PORT_SIZE);
    if (!GLUtils::CheckError(__FILE__, __LINE__)) {
        EFFECT_LOGE("GpuContrastFilterOperator Render Failed");
    ...
    if (!GLUtils::CheckError(__FILE__, __LINE__)) {
        EFFECT_LOGE("GpuContrastFilterOperator Render Read Pixels Failed");
...
ErrorCode GpuContrastFilterOperator::OnApplyYUVNV21(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_LOGI("GpuContrastFilterOperator::OnApplyYUVNV21 enter!");
    return ErrorCode::SUCCESS;
}

ErrorCode GpuContrastFilterOperator::OnApplyYUVNV12(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_LOGI("GpuContrastFilterOperator::OnApplyYUVNV12 enter!");
    return ErrorCode::SUCCESS;
}
} // namespace Effect
} // namespace Media
