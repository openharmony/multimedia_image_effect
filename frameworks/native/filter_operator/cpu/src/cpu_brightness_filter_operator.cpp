...
#include "cpu_brightness_filter_operator.h"
...
#include "filter_operator_utils.h"
#include "securec.h"
...
namespace Media {
namespace Effect {
REGISTER_FILTER_OPERATOR(Brightness, CpuBrightnessFilterOperator);
const std::vector<IEffectFormat> CpuBrightnessFilterOperator::FORMAT_TYPES = {
...
ErrorCode CpuBrightnessFilterOperator::OnApplyRGBA8888(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_LOGI("CpuBrightnessFilterOperator::OnApplyRGBA8888 enter!");
...
ErrorCode CpuBrightnessFilterOperator::OnApplyYUVNV21(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_TRACE_NAME("CpuBrightnessFilterOperator::OnApplyYUVNV21");
    EFFECT_LOGI("CpuBrightnessFilterOperator::OnApplyYUVNV21 enter!");
    CHECK_AND_RETURN_RET_LOG(src != nullptr && dst != nullptr, ErrorCode::ERR_INPUT_NULL,
        "input para is null! src=%{public}p, dst=%{public}p", src, dst);
    float brightness = ParseBrightness(value);
...
#pragma omp parallel for default(none) shared(height, width, srcNV21, dstNV21, lut)
    for (uint32_t i = 0; i < height; i++) {
        for (uint32_t j = 0; j < width; j++) {
            ...
            uint8_t r = FilterOperatorUtils::YuvToR(y, u, v);
            uint8_t g = FilterOperatorUtils::YuvToG(y, u, v);
            uint8_t b = FilterOperatorUtils::YuvToB(y, u, v);
            r = lut[r];
            g = lut[g];
            b = lut[b];
            dstNV21[y_index] = FilterOperatorUtils::RGBToY(r, g, b);
            dstNV21UV[nv_index] = FilterOperatorUtils::RGBToV(r, g, b);
            dstNV21UV[nv_index + 1] = FilterOperatorUtils::RGBToU(r, g, b);
        }
    }
...
ErrorCode CpuBrightnessFilterOperator::OnApplyYUVNV12(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_TRACE_NAME("CpuBrightnessFilterOperator::OnApplyYUVNV12");
    EFFECT_LOGI("CpuBrightnessFilterOperator::OnApplyYUVNV12 enter!");
    CHECK_AND_RETURN_RET_LOG(src != nullptr && dst != nullptr, ErrorCode::ERR_INPUT_NULL,
        "input para is null! src=%{public}p, dst=%{public}p", src, dst);
    float brightness = ParseBrightness(value);
...
#pragma omp parallel for default(none) shared(height, width, srcNV12, dstNV12, lut)
    for (uint32_t i = 0; i < height; i++) {
        for (uint32_t j = 0; j < width; j++) {
            ...
            dstNV12[y_index] = FilterOperatorUtils::RGBToY(r, g, b);
            dstNV12UV[nv_index] = FilterOperatorUtils::RGBToU(r, g, b);
            dstNV12UV[nv_index + 1] = FilterOperatorUtils::RGBToV(r, g, b);
        }
    }
    return ErrorCode::SUCCESS;
}

float CpuBrightnessFilterOperator::ParseBrightness(std::map<std::string, Plugin::Any> &value)
{
