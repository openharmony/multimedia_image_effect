...
#include "cpu_contrast_filter_operator.h"
...
namespace Media {
namespace Effect {
REGISTER_FILTER_OPERATOR(Contrast, CpuContrastFilterOperator);
const std::vector<IEffectFormat> CpuContrastFilterOperator::FORMAT_TYPES = {
...
{
    EFFECT_LOGI("CpuContrastFilterOperator::OnApplyRGBA8888 enter!");
...
ErrorCode CpuContrastFilterOperator::OnApplyYUVNV21(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_LOGI("CpuContrastFilterOperator::OnApplyYUVNV21 enter!");
    CHECK_AND_RETURN_RET_LOG(src != nullptr && dst != nullptr, ErrorCode::ERR_INPUT_NULL,
        "input para is null! src=%{public}p, dst=%{public}p", src, dst);
    float contrast = ParseContrast(value);
...
ErrorCode CpuContrastFilterOperator::OnApplyYUVNV12(EffectBuffer *src, EffectBuffer *dst,
    std::map<std::string, Plugin::Any> &value)
{
    EFFECT_TRACE_NAME("CpuContrastFilterOperator::OnApplyYUVNV12");
    EFFECT_LOGI("CpuContrastFilterOperator::OnApplyYUVNV12 enter!");
    CHECK_AND_RETURN_RET_LOG(src != nullptr && dst != nullptr, ErrorCode::ERR_INPUT_NULL,
        "input para is null! src=%{public}p, dst=%{public}p", src, dst);
    float contrast = ParseContrast(value);
...
float CpuContrastFilterOperator::ParseContrast(std::map<std::string, Plugin::Any> &value)
{
    float contrast = 0.f;
